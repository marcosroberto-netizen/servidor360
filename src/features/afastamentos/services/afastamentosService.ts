...
function buildDocumentoName(servidor: ServidorOption | undefined, tipo: string, file: File) {
...
function mapAfastamento(row: AfastamentoRow, servidor: ServidorOption | undefined): AfastamentoResumo {
  return {
    id: row.id,
    ...
    documentoOrigemUrl: row.documento_origem_url,
...
async function getServidoresById(ids: string[]) {
  ...
  const unidadeIds = [...new Set((data ?? []).map((servidor) => servidor.unidade_id))]
  const { data: unidades, error: unidadesError } = unidadeIds.length > 0
    ? await supabase
  ...
  const unidadeNamesById = new Map((unidades ?? []).map((unidade) => [unidade.id, unidade.nome]))
  ...
  for (const servidor of data ?? []) {
    servidores.set(servidor.id, {
      ...
      unidadeNome: unidadeNamesById.get(servidor.unidade_id) ?? 'Unidade nao informada',
...
export async function createAfastamento(input: AfastamentoFormData): Promise<string> {
  ...
  const uploadedPath = input.documentoArquivo
    ? await uploadDocumento(input.documentoArquivo, `origem/${input.servidorId}`, documentoNome ?? input.documentoArquivo.name)
    : null
...
export async function listAfastamentos(): Promise<AfastamentoResumo[]> {
  const { data, error } = await supabase
    .schema('afastamentos')
    .from('afastamentos')
    .select('id, servidor_id, status, protocolo, tipo, data_inicio, data_fim, motivo, documento_origem_nome, documento_origem_url, iniciado_em, enviado_em')
    .order('updated_at', { ascending: false })

  if (error) throw error

  const rows = (data ?? []) as AfastamentoRow[]
  const servidores = await getServidoresById(rows.map((row) => row.servidor_id))

  return rows.map((row) => mapAfastamento(row, servidores.get(row.servidor_id)))
}

export async function getAfastamentoDetalhe(id: string): Promise<AfastamentoDetalhe> {
  const { data, error } = await supabase
    .schema('afastamentos')
    .from('afastamentos')
    .select('id, servidor_id, status, protocolo, tipo, data_inicio, data_fim, motivo, observacoes, documento_origem_nome, documento_origem_url, iniciado_em, enviado_em')
    .eq('id', id)
    .single()

  if (error) throw error

  const row = data as AfastamentoRow
  const servidores = await getServidoresById([row.servidor_id])
  const signedDocumentoUrl = await createSignedDocumentoUrl(row.documento_origem_url)
  ...
  const [
    ...
    supabase
      .schema('afastamentos')
      .from('movimentacoes')
      .select('id, tipo, titulo, descricao, status_origem, status_destino, criado_em')
      .eq('afastamento_id', id)
      .order('criado_em', { ascending: false }),
    supabase
      .schema('afastamentos')
      .from('complementacoes')
      .select('id, solicitacao, resposta, documento_nome, documento_url, solicitada_em, respondida_em, status')
      .eq('afastamento_id', id)
...
export async function responderComplementacao(input: ResponderComplementacaoInput) {
  ...
  const documentoNome = input.documentoArquivo
    ? buildDocumentoName(servidor, `${afastamentoData.tipo ?? 'afastamento'}-complementacao`, input.documentoArquivo)
    : null
  const uploadedPath = input.documentoArquivo
...
export async function listDevolutivaAlerts(): Promise<DevolutivaAlert[]> {
  ...
  return processosComDevolutiva.slice(0, 5).map((item) => ({
    ...
    recebidaEm: item.enviadoEm ?? item.iniciadoEm,
